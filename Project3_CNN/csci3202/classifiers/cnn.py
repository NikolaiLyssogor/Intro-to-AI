...
class ThreeLayerConvNet(object):
  ...
  def loss(self, X, y=None):
    ...
    # variable.                                                                #
    ############################################################################
    # conv_relu_pool -> affine_relu -> affine
    ...
    dx3, grads['W3'], grads['b3'] = affine_backward(soft_dx, cache3)
    dx2, grads['W2'], grads['b2'] = affine_relu_backward(dx3, cache2)
    dx1, grads['W1'], grads['b1'] = conv_relu_pool_backward(dx2, cache1)
    # add l2 regularization term to the loss for each weight vector
    loss += self.reg * (np.square(W1).sum() + np.square(W2).sum() + np.square(W3).sum())
    # add l2 regularization term to each of the gradients
