...
def one_dimensional_curve_fitting_test(lambda_fun, x_limits, n_data_points, pop_size = 1000, num_iters = 100, n_test_points = 100, method='ga'):
    ...
    if method == 'ga':
        ...
        for i in range(20000):
            expr = generate_random_expr(params.depth, ['x'], params)
            if(is_viable_expr(expr, ['x'], params)):
                current_fitness = compute_fitness(expr, ['x'], params)
    ...
    plt.show()
    if method == 'ga':
        return (best_fitness, best_expr, start_time - time.time())
    else:
